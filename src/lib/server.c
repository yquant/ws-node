...
int wsn_server_init(wsn_server_ctx_t *server, wsn_server_conf_t *conf, uv_loop_t *loop)
{
  server->conf = conf;
...
static void on_new_connection_(uv_stream_t *stream, int status)
{
  ...
  conn = wsn_conn_create(listen_ctx, WSN_CONN_DIRECTION_IN);
  if (conn) {
...
static int wsn_server_start_listen_(wsn_server_listen_ctx_t *listen_ctx, uv_loop_t *loop)
{
  ...
  if (err) {
    ...
    wsn_report_err(WSN_ERR_BIND, "Bind failed for (\"%s\"): %s",
  ...
  if (err) {
    wsn_server_listen_ctx_cleanup(listen_ctx);
    wsn_report_err(WSN_ERR_LISTEN, "Listen failed for (\"%s\"): %s",
                   listen_ctx->server->conf->host, uv_strerror(err));
...
static void on_get_server_addrs_(uv_getaddrinfo_t *req, int status, struct addrinfo *addrs)
{
  ...
  if (status < 0) {
    wsn_report_err(WSN_ERR_GET_ADDR_INFO, "Failed to getaddrinfo for (\"%s\"): %s",
  ...
  if (ipv4_naddrs == 0 && ipv6_naddrs == 0) {
    wsn_report_err(WSN_ERR_GET_ADDR_INFO, "(\"%s\") has no IPv4/6 addresses", server->conf->host);
    uv_freeaddrinfo(addrs);
    return;
  }
  ...
  if (server->listen_ctxs == NULL) {
    wsn_report_err(WSN_ERR_MALLOC, "Malloc listen contexts for (\"%s\") failed",
                   server->conf->host);
    uv_freeaddrinfo(addrs);
    return;
  }
...
int wsn_server_start(wsn_server_ctx_t *server)
{
  ...
  err = uv_getaddrinfo(server->loop,
                       ...
                       &hints);
  if (err != 0) {
    wsn_report_err(WSN_ERR_GET_ADDR_INFO, "Failed to start server %p, getaddrinfo error: %s", uv_strerror(err));
